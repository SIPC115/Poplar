...
# 层叠顺序  
上面两个概念都是为了要解决这么一个问题，当元素发生层叠时到底在垂直方向上是怎样的显示顺序呢？层叠上下文和层叠水平是概念，而这里的层叠顺序是规则。。
...
* 当具有明显的层叠水平标识的时候，如识别的z-indx值，在同一个层叠上下文领域，层叠水平值大的那一个覆盖小的那一个。
