...
## height: 100%  
有时候我们想让元素height像width一样能够充满整个屏幕，设置`height: 100%;`并不能起到任何作用。原因就在于，height的默认高度是由内容决定的。所以
...
## auto高度  
在最简单的情况下，如果块级正常流元素设置为`height:auto`, 显示时其高度将恰好足以包含其内联内容(包括文本)的行盒。高度auto时，会在段落上设置
一个边框，并认为没有内边距，这样下边框刚好在文本最后一行的下面，上边框则刚好在文本第一行的上面。 
如果块级元素正常流元素的高度设置为auto，并且只有块级子元素，其默认高度将是从最高块级子元素的外边框边界到最低块级子元素外边框边界之间的距离。因此，
自匀外边距会“超出”包含这些子元素的元素。不过，块级元素有内边距或者边框或者设置BFC了，那么其高度则从其最高子元素的上外边距边界到其最低子元素的下外边距
边界之间的距离。如下：  
...
```   
![](/image/css2-6.png)  
蓝色块，本来是想相对于绿色块上边距为10px。但是我们根本看不见绿色块，因为绿色块和蓝色块本计算成了一体。我们在绿色块上加上`overflow: hidden;`如下：  
```html  
<div style="border: 1px solid #ff0000;">
...
![](/image/css2-7.png)  
这次就变成了我们想要的结果。当然你也同样可以在绿色块上加入内边距或者边框。  
...
```  
![](/image/css2-9.png)   
行框的宽度由它的包含块和其中的浮动元素决定。高度由行高度计算规则决定。  
...
一个浮动的盒子将会一直向左或向右移动，一直到触碰到包含块的边界或者其他浮动盒子的外边界。如果是行盒子的话，那么浮动盒子的外边界的顶部将与当前行盒子的顶部对齐。
