...
从右想做匹配这一点很关键，Sizzle引擎也是这样设计的。之后会细说。  
### css执行效率  
...
* 对于选择器中给定的各个类属性值，属性选择或伪类，加: 0, 0, 0, 1
...
但有时为了某个生命特别重要，超过了其它生命。CSS2.1称之为重要声明，并允许在这些声明的结束分好之前插入`!important`来标志。这个规则永远排在最前边。   
...
4. 案出现顺序对应用到给定元素的所有声明排序，一个声明在样式表或文档中越后出现，他的权重越大。  
